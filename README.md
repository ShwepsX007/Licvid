...
- **Регулируемые панели, как на биржах.** Между графиком и лентой есть
  ...
  F5; на телефоне тоже есть разделители: между графиком и лентой (высота
  графика 20–88% экрана, лента сжимается парой) и высота блока «Лидеры».
  Ручки крупные, с захватом указателя и touch-фолбэком для старых браузеров.
- **CVD-шарики: кто двигает цену.** У каждой свечи считается тейкер-дельта —
  ...
  **OKX** (публичная статистика `rubik/taker-volume-contract`); у Bybit
  ...
  используется.
- **Три слоя на графике, кнопки в легенде.** «⚡ Ликвидации» — прямоугольники
  ликвидаций (и маркеры китов), «📊 Профиль» — профиль объёмов по ценам,
  «🎯 CVD» — шарики. Состояние каждой кнопки сохраняется в браузере
