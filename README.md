...
- **Шарики ликвидаций** рисуются прямо на свече, в точке цены события, и
  прижимаются к диапазону `low..high` этой свечи. Биржи отдают цену
  банкротства — она может уходить далеко от рынка, поэтому без ограничения
  шарики «улетали» в пустое поле графика. Если свечи за это время в серии
  нет, событие не рисуется вовсе.
- Палитра намеренно не повторяет цвета свечей: ликвидация лонгов —
  **маджента**, шортов — **циан**, кит от $100k — **золото** с белым ободком
  и тёмной подложкой, чтобы шарик читался внутри тела свечи.
  Стрелки над/под свечой остались только для китов.
...
- **Телефон**: раскладка перестраивается в одну колонку (раньше лента шириной
  ...
  «Биржа» и «Цена».
- **Наведение на шарик подсвечивает ленту.** Каждый шарик — это кластер
  ликвидаций одной свечи на одном ценовом уровне (внутри свечи 6 уровней).
  Наведите курсор на шарик — в «Прямом эфире» контрастно подсветятся именно
  те события, из которых шарик собран (остальное затемняется), и лента
  прокрутится к первому. Клик (на телефоне — тап) закрепляет подсветку,
  повторный клик/тап по тому же шарику или по пустому месту — снимает.
...
- **CVD-стрелки: кто двигает цену.** У каждой свечи считается тейкер-дельта —
  разница объёмов агрессивных покупок (бьют по аску) и агрессивных продаж
  (бьют по биду) от открытия до закрытия свечи, в USDT. Зелёная ▲ над свечой —
  в ней преобладали покупатели, красная ▼ под свечой — продавцы; размер
  стрелки — величина перевеса. На формирующейся свече стрелка живая (обновляется
  на каждом тике), после закрытия остаётся с финальным значением. Рядом с
  кнопкой — индикатор перевеса текущей свечи («▼ −$158K»). Источники истории
  ...
  **OKX** (публичная статистика `rubik/taker-volume-contract`); у Bybit
  ...
  недоступен, дельта текущей свечи всё равно считается из тиков — стрелки
  растут на глазах. Поле `cvd_source` в `/api/status` показывает, чья история
  используется.
- **Три слоя на графике, кнопки в легенде.** «⚡ Ликвидации» — шарики
  ликвидаций (и маркеры китов), «📊 Профиль» — профиль объёмов по ценам,
  «🎯 CVD» — стрелки. Состояние каждой кнопки сохраняется в браузере
...
- **История пузырьков переживает F5 и рестарт сервера.** Сервер пишет каждое
  ...
  собственная история. Видимое окно шариков по-прежнему ограничено
