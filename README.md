...
- **CVD-шарики: кто двигает цену.** У каждой свечи считается тейкер-дельта —
  разница объёмов агрессивных покупок (бьют по аску) и агрессивных продаж
  (бьют по биду) от открытия до закрытия свечи, в USDT. Шарик стоит строго
  в центре тела свечи: **фиолетовый ▲** — преобладали покупатели,
  **оранжевый ▼** — продавцы; размер шарика и подпись суммы — величина
  перевеса (мелочь рисуется без подписи, слабые сигналы при тесноте
  уступают место сильным). На формирующейся свече шарик живой (обновляется
  ...
  **OKX** (публичная статистика `rubik/taker-volume-contract`); у Bybit
  ...
  недоступен, дельта текущей свечи всё равно считается из тиков — шарики
  растут на глазах. Поле `cvd_source` в `/api/status` показывает, чья история
  используется.
- **OI-треугольники: куда идут деньги.** У каждой свечи считается изменение
  открытого интереса за её время (USD, сумма по биржам с историей).
  Рост — треугольник **teal ▲ над хаем**, падение — **rose ▼ под лоем**,
  с отступом от свечи. Размер и яркость — по величине изменения
  (цифры для BTC): мелочь **<$1M — мини-значок без цифр**, $1M+ —
  подписанный треугольник, $5M+ и $20M+ — всё крупнее, ярче (заливка
  осветляется) и с более сильным свечением; слабые при тесноте уступают
  сильным. Для монет с меньшим оборотом тиры ниже (см. ниже). В шапке —
  бокс OI
  по монете графика:
  ...
  **Gate** (`contract_stats`); изменения и треугольники считаются только
...
- **Четыре слоя на графике, кнопки в легенде.** «⚡ Ликвидации» —
  ...
  объёмов по ценам, «🎯 CVD» — шарики, «▲ OI» — треугольники открытого
...
python3 tests/test_oi.py         # открытый интерес: 7 бирж, бакеты, треугольники, лидеры
