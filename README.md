...
| `GET /api/stats?symbol=` | агрегаты: лонги/шорты за 5м/1ч/24ч, топ монет, разрез по биржам |
| `GET /api/oi?symbol=BTC_USDT` | открытый интерес: текущий тотал по живым ногам + изменения m5/h1/h24 |
...
- **CVD-шарики: кто двигает цену.** У каждой свечи считается тейкер-дельта —
  ...
  **оранжевый ▼** — продавцы; размер шарика и подпись суммы — величина
  ...
  табло CVD 24ч/1ч/5м по монете графика, рядом с кнопкой — индикатор
...
- **OI-треугольники: куда идут деньги.** У каждой свечи считается изменение
  ...
  внутри — сумма изменения (мелочь — мини-значок без цифр, слабые при
  тесноте уступают сильным). В шапке — табло OI по монете графика:
  текущий интерес со всех 7 бирж + изменения 24ч/1ч/5м. Источники:
