    ...
    function renderAiPrompts(d) {
        ...
        box.innerHTML = (d.blocks || []).map(function (b) {
            return '<div class="ai-block"><h4>' + esc(b.title) + "</h4>" +
                (b.kind === "digest" ? ("<p class=\"meta\">Фото к посту крепится, только если весь пост " +
                    "(шапка + рассказ + сухие блоки + ссылка) уложится в <b>1024 знака</b> " +
                    "— больше Telegram не разрешает в подписи под фотографией. Сухие блоки " +
                    "занимают 600–1300 знаков в зависимости от дня: в спокойный день рассказу " +
                    "остаётся ~400 знаков, а в полный (с блоком OI) подпись переполнена даже " +
                    "без рассказа. Пост длиннее лимита уходит текстом, без фото; рассказ " +
                    "при этом не подрезается — длину держите промтом.</p>") : "") +
