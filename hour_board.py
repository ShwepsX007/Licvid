...
Зачем отдельный модуль: сводка собиралась из буфера LIQUIDATIONS, но он
кольцевой и при рестарте теряет прошлое. Стенду нужны календарные часы за
всё окно поста (до 24) — их и копим здесь, а не выпрашиваем у буфера.
...
#: Базовая сетка для постов в канал. 15 минут: почасовый блок поста — ровно
#: четыре слота (1 час), а сама сетка при смене частоты не меняется, поэтому
#: уже собранные данные никуда не пропадают.
SLOT_SEC = 900
#: Сколько базовых слотов держать: максимальная частота постов — раз в 24 ч,
#: на окно и предыдущее окно нужно 2×24 ч, плюс запас на текущий блок.
KEEP_SLOTS = (2 * 24 + 2) * (HOUR // SLOT_SEC)
...
def slot_hhmm(ts: float, tz: int = 0) -> str:
    """«ЧЧ:ММ» — подпись слота постов.

    У постов в канал блок — ровно час, поэтому в подписи всегда «ЧЧ:00»;
    функция оставлена общей для случаев, когда блок короче часа.
    """
    lt = time.gmtime(float(ts) + tz)
    ...
    def _cell(self, ts: float) -> Optional[dict]:
        ...
        if cell is None:
            ...
            cell = {"h": h, "total": 0.0, "longs": 0.0, "shorts": 0.0, "count": 0,
                    "coins": {}, "cnt": {}, "cvd": {}, "top": []}
            self._hours[h] = cell
            # старые часы больше не нужны: сравнение идёт максимум на
            # два окна назад (окно — до 24 часов)
...
def fold_slots(cells: List[dict], group: int = 1) -> List[dict]:
    ...
    Нужно постам в канал: блок поста — ровно 1 час (4 слота), а копятся
    данные всегда на одной сетке — 15 минут. Блок получает время первого
    своего слота, поэтому подпись «🕘 18:00» остаётся честной.
    """
...
def build_snapshot(board: "HourBoard", oi: "OiHistory", now: Optional[float] = None,
    ...
    блок поста. Сводка почасовая: блок — ровно 1 час (``group = HOUR //
    board.slot_sec``), а ``span`` — число часов в окне, равное частоте постов:
    пост раз в N часов разбирает N последних часов. Данные копятся на сетке
    15 минут (``SLOT_SEC``), поэтому смена частоты не рвёт собранную историю.
    При почасовом блоке окно притягивается к завершённым календарным часам —
    см. выравнивание ``end`` ниже.
    """
    now = float(now if now is not None else time.time())
    group = max(1, int(group))
    base = max(60, int(getattr(board, "slot_sec", HOUR) or HOUR))
    block_sec = base * group
    # Почасовая сетка (блок = ровно 1 час): окно — завершённые календарные
    # часы; последний показанный час — последний завершённый, а не идущий
    # прямо сейчас. Пост в 13:07 разбирает часы 12, 11, 10… — цифры каждого
    # часа уже финальные, «живой» час в сводку не попадает.
    if block_sec == HOUR:
        end = hour_start(now, board.tz) - 1
    else:
        end = now
    tail = fold_slots(board.slots((span + 1) * group, end), group)   # +1 блок — база
    hours = tail[-span:] if len(tail) > span else tail
    prev = fold_slots(board.slots(span * group, end - span * block_sec), group)
...
BOARD = HourBoard()
OI = OiHistory()
#: Доска постов в канал: сетка 15 минут. Из неё собираются почасовые блоки
#: поста — один блок ровно 1 час (четыре слота), блоков — по числу часов окна.
