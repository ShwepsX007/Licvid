...
CHANNEL_LIMIT = 950             # пост в канале: лимит подписи 1024, держим запас
...
def lead_of(narrative: str, budget: int) -> str:
    ...
    Полный разбор живёт на сайте, а в канал уходит небольшой пост: под
    фотографией Telegram пускает не больше 1024 знаков, и лид приходится
    сокращать. Режем **по границе предложения** и добавляем «…» — читатель
    видит, что дальше есть текст, а ссылка под постом ведёт на полный разбор.
    """
